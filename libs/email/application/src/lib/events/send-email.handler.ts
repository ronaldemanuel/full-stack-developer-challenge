...
export namespace SendEmailEventHandler {
  export const TOKEN = 'SendEventEmailHandler';
  @EventsHandler(SendEmailEvent)
  export class Handler implements IEventHandler<SendEmailEvent<EmailTypes>> {
    constructor(
      private readonly nodemailer: MailerService,
      private readonly renderEmailService: EmailRenderService.Service,
    ) {}

    private readonly logger = new Logger('SendEmailEventHandler');
